...
class Generator(nn.Module):
    def __init__(self, input_dim=100, output_channel=1):
        super(Generator, self).__init__()
        self.model = nn.Sequential(
            nn.ConvTranspose2d(input_dim, 512, kernel_size=3, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(512),
            nn.ReLU(True),
            ...
            nn.BatchNorm2d(128),
            nn.ReLU(True),
            nn.ConvTranspose2d(128, output_channel, kernel_size=4, stride=2, padding=1, bias=False),
            nn.Tanh()
        )
...
if __name__ == '__main__':
    ...
    datetime_str = current_datetime.strftime("dcgan_%Y-%m-%d_%H-%M-%S")
